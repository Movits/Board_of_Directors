...
Apresente uma ideia e um conselho de **13 conselheiros de IA** (cada um com experiência, personalidade e opiniões próprias) analisa, propõe estratégias, **debate em rodadas** e **vota** na decisão. No final, a Presidente do Conselho junta tudo em um veredito e entrega:
...
- **Pitch rico:** além do texto, anexe **imagens, PDFs e arquivos de texto** (identidade visual, mockups, pesquisa), **conecte um repositório do GitHub** ou **analise uma pasta local do seu computador** (para projetos ainda não publicados; o navegador lê o código aí mesmo). Nos três casos os conselheiros recebem um resumo do código (árvore de arquivos, README, dependências, trechos) e analisam o estado REAL do projeto. A pasta local só sai do navegador ao ir para o provedor de IA na análise. Com *Rodar de graça no seu chat de IA*, nem isso.
...
4. **Entregáveis** (opcionais): 📄 o **plano detalhado** em documento visual com download em PDF (análise de mercado, SWOT, cronograma, orçamento com gráficos, riscos), e 🚀 o **prompt de execução** autossuficiente para colar num agente de programação. O conselho decide; quem executa é você (ou o seu agente).

No debate, cada conselheiro tem voz e opiniões próprias e é incentivado a **discordar de forma produtiva** (não a concordar por conformismo). Se a síntese falhar por algum motivo, as análises e o debate **não se perdem**: dá para gerar a síntese de novo. Ao final, com uma API conectada, a sala mostra **quanto a reunião custou de verdade** (a partir dos tokens consumidos) e você pode **interromper** uma reunião em andamento.
...
O provedor **Personalizado** conecta qualquer API compatível com o protocolo da OpenAI: **Ollama no seu computador**, LM Studio, OpenRouter, Groq, Gemini, Mistral, vLLM… Basta informar a **Base URL** (há atalhos prontos), opcionalmente a chave, e usar o botão **"🔎 Buscar modelos"** para listar os modelos disponíveis na própria API. Para APIs que não suportam saída estruturada, a app cai automaticamente para instruções de JSON no prompt.
...
Sem chave, a app roda em **modo de teste** (respostas de exemplo, sem custo) para você conhecer a interface, ou clique em **"Ver reunião de exemplo"** para o conselho decidir uma ideia curada em **um clique**, sem digitar nada. O modo de teste é uma simulação honesta: o placar bate com o veredito e há divergência real na mesa (não uma unanimidade encenada).
...
Não quer pagar nada? Na tela inicial (e na página do projeto) há o botão **🖥 Rodar de graça**: ele gera um **prompt pronto** para você colar no seu chat de IA (ChatGPT, Claude, Gemini, Copilot e outros). O chat convoca os 13 conselheiros, faz o debate, a votação e entrega veredito, plano e próximos passos, **rodando no plano que você já paga, sem custo**. Como bônus, em assistentes que mexem em código (como o [Claude Code](https://claude.com/claude-code) ou o Cursor), ele ainda lê o seu projeto de verdade e pode **criar as coisas por você**.
...
> **Por que o app não usa o seu plano direto?** Os planos dos chats de IA não deixam apps de fora usarem a sua conta automaticamente. Por isso o caminho é o app gerar um prompt para você mesmo colar no seu chat, onde ele roda de graça. A chave de API é um caminho à parte e pago.

### Plano do chat de IA ≠ chave de API

A chave de API (console.anthropic.com) usa **créditos próprios, pagos por consumo**. Ela é separada do seu plano do Claude (Pro/Max) e **não consome o limite do plano**. Para rodar no app com custo mínimo: compre um valor pequeno de créditos (ex.: US$5), defina limite de gasto e use **Sonnet ou Haiku**. Para custo **zero** usando o plano que você já tem, veja *Rodar de graça no seu chat de IA* acima.
